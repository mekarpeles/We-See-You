...
#
# Modified from an original version written by Matt Wood <http://github.com/mattwood>
#
...
class EnvolveAPIKey():
    ...
    def __init__(self, api_key):
        ...
        self.full_key = '{0:d}-{1}'.format(self.site_id, self.secret_key)
...
def _wrap_command(api_key, command):
    ...
    return hmac.new(api_key.secret_key, command_string, hashlib.sha1).hexdigest() + ';' + command_string
...
def get_login_command(envolve_api_key, first_name,
                        last_name=None, pic=None, is_admin=False, profHTML=None):
    """Returns the hashed logout command string for use in the javascript call
    to the Envolve API.
    ...
    is_admin -- optional boolean for the user's admin status.
                Default value: False.
    profHTML -- optional HTML to be inserted into a user's profile rollover
                Default value: None.
    """
    api_key = EnvolveAPIKey(envolve_api_key)
    if not first_name:
        raise EnvolveAPIException("""You must provide at least a first name.
                                     If you are providing a username, use it
                                     for the first name.""")
...
def get_html(envolve_api_key, first_name=None, last_name=None,
    ...
    """Returns the javascript tags necessary to use the Envolve API login
    mechanism.
    ...
    is_admin -- optional boolean for the user's admin status.
                Default value: False.
    profHTML -- optional HTML to be inserted into the user's profile rollover
                Default value: None.

    If first_name is not passed in, the user will be anonymous.

    To use, import this module and call the get_html function
    with the appropriate and correct keywords specified.  The function will
    return javascript that you can use in your page's HTML as you build your
    GET response.
    ...
    if first_name:
        ...
        return _get_html_for_command(api_key, get_logout_command(api_key.full_key))
